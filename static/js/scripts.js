document.addEventListener("DOMContentLoaded", async (event) => {
    ...
    const serverSelect = document.getElementById("server-select"); // Added server selection element
    const buyMatsSelect = document.getElementById("buy-mats"); // Added buy-mats select element
    ...
    serverSelect.addEventListener("change", fetchAndDisplayResults); // Fetch results on server change
    buyMatsSelect.addEventListener("change", fetchAndDisplayResults); // Fetch results on buy-mats location change

    function updateItemImage() {
        // The <option> value is the display name; the API needs the item key
        // (e.g. "MAIN_ARCANESTAFF") with a tier prefix.
        const selectedOption = itemSelect.options[itemSelect.selectedIndex];
        const itemKey = selectedOption ? selectedOption.getAttribute("data-key") : "";
        const fullItemId = "T4_" + itemKey;
        const baseUrl = "https://render.albiononline.com/v1/item/";

        itemImage.src = baseUrl + fullItemId;
        itemImage.alt = fullItemId;
        itemImage.title = fullItemId;
    }

    function updateStationFee() {
        const fee = parseFloat(stationFeeInput.value);
        if (isNaN(fee) || fee < 0) {
    ...
    function updateReturnRate() {
        ...
        baseRate = parseFloat(baseRate.replace(',', '.')) / 100;

        if (dailyBonus !== "None") {
            const dailyBonusRate = parseFloat(dailyBonus.replace('%', '')) / 100;
            baseRate = baseRate * (1 + dailyBonusRate);
        }

        returnRateInput.value = (baseRate * 100).toFixed(2) + "%";
    }

    function getItemValue(itemId, tier) {
        const itemCategories = {
            "shoes_or_helmet": /SHOES|HEAD|OFF/,
            "shoes_or_helmet_rune": /SHOES|HEAD.*KEEPER|OFF.*KEEPER/,
            "shoes_or_helmet_soul": /SHOES|HEAD.*HELL|OFF.*HELL/,
            "shoes_or_helmet_relic": /SHOES|HEAD.*MORGANA|OFF.*MORGANA/,
            "shoes_or_helmet_avalon": /SHOES|HEAD.*AVALON|OFF.*AVALON/,
            "1h_weapon": /1H|MAIN/,
            "1h_weapon_rune": /1H.*KEEPER|MAIN.*KEEPER/,
            "1h_weapon_soul": /1H.*HELL|MAIN.*HELL/,
            "1h_weapon_relic": /1H.*MORGANA|MAIN.*MORGANA/,
            "1h_weapon_avalon": /1H.*AVALON|MAIN.*AVALON/,
            "2h_weapon": /2H/,
        ...
        // Manual override for MAIN_RAPIER_MORGANA
        ...
        if (manualOverrides[itemId]) {
            return manualOverrides[itemId][tier] || null;
        }
    ...
    async function fetchAndDisplayResults() {
        ...
        const selectedServer = serverSelect.value; // Get selected server
        const selectedBuyMatsLocation = buyMatsSelect.value; // Get selected buy-mats location
        ...
        for (const batch of batchRequests) {
            const url = `https://${selectedServer}.albion-online-data.com/api/v2/stats/prices/${batch.join(',')}?locations=${selectedCity}&qualities=${selectedQualityNumber}`;
        ...
        calculateAndDisplayProfit(allData, selectedItemId, selectedQuantity, stationFee, selectedCity, selectedBuyMatsLocation, tierItemValues);
    }

    async function calculateAndDisplayProfit(data, selectedItemId, quantity, stationFee, city, buyMatsLocation, tierItemValues) {
        ...
        for (const material of materials) {
            ...
            const url = `https://${selectedServer}.albion-online-data.com/api/v2/stats/prices/${materialRequestString}?locations=${buyMatsLocation}`;
